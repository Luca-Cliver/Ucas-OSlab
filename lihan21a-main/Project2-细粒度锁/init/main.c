...
static void init_task_info(void)
{
    ...
    bios_sd_read(0x54000000, 2, taskinfo_blockid);

    task_info_t *cur_task = (task_info_t *)0x54000000;
...
static void init_pcb(void)
{
    ...
    spin_lock_init(&pid0_pcb.spinlock);
    int j = 0;
    for(int i = 0; i < 21; i++)
    {
        if(i == 0)
...
int main(void)
{
    if(get_current_cpu_id() != 0)
    {
        //拿锁，初始化
        lock_kernel();
        //newcore_pcb = &pid0_pcb_promax;
        ptr_t kernel_stack = allocKernelPage(1)+PAGE_SIZE;
        //ptr_t kernel_stack = INIT_KERNEL_STACK_SLAVE + PAGE_SIZE;
        ...
        spin_lock_init(&newcore_pcb->spinlock);
