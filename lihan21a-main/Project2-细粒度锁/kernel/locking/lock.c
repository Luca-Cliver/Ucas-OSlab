...
void do_mutex_lock_acquire(int mlock_idx)
{
    ...
    {
        do_block(&(current_running[cpu_id]->list), &(mlocks[mlock_idx].block_queue), &mlocks[mlock_idx].lock);
    }
...
int do_barrier_init(int key, int goal)
{
    ...
    spin_lock_acquire(&(barrier[barrier_id].lock));

    barrier[barrier_id].goal = goal;
    barrier[barrier_id].now_num = 0;
    list_init(&(barrier[barrier_id].block_queue));

    spin_lock_release(&(barrier[barrier_id].lock));
    return barrier_id; 
}

void do_barrier_wait(int bar_idx)
{
    spin_lock_acquire(&(barrier[bar_idx].lock));
    uint64_t cpu_id = get_current_cpu_id();
    ...
    {
        do_block(&(current_running[cpu_id]->list), &barrier[bar_idx].block_queue, &barrier[bar_idx].lock);
    }
    spin_lock_release(&(barrier[bar_idx].lock));
}
...
int do_semaphore_init(int key, int init)
{
    ...
    spin_lock_acquire(&(sema[sema_id].lock));
    sema[sema_id].value = init;
    list_init(&(sema[sema_id].block_queue));
    spin_lock_release(&(sema[sema_id].lock));
    return sema_id;
}
...
void do_semaphore_down(int sema_idx)
{
    ...
    {
        do_block(&(current_running[get_current_cpu_id()]->list), &(sema[sema_idx].block_queue), &sema[sema_idx].lock);
    }
...
int do_mbox_send(int mbox_idx, void *msg, int msg_length)
{
    ...
    {
        ret++;
        do_block(&(current_running[get_current_cpu_id()]->list), &(mbox[mbox_idx].block_queue), &mbox[mbox_idx].lock);
    }
