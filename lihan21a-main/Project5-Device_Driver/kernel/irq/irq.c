...
void handle_irq_ext(regs_context_t *regs, uint64_t stval, uint64_t scause)
{
    ...
    int claim_idx = plic_claim();
    //printk("claim = %d\n", claim_idx);
    //assert(claim_idx == PLIC_E1000_QEMU_IRQ);
    //assert(claim_idx == PLIC_E1000_PYNQ_IRQ);
    if(claim_idx == PLIC_E1000_PYNQ_IRQ)
    {
        net_handle_irq();
    }

    plic_complete(claim_idx);
}
