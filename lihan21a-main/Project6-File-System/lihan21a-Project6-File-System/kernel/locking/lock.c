...
{
    lock_zero.lock.status = UNLOCKED;
    list_init(&lock_zero.block_queue);
...
void do_mutex_lock_acquire(int mlock_idx)
{
    ...
    if(mlocks[mlock_idx].lock.status == UNLOCKED)
    {
        ...
        mlocks[mlock_idx].lock.owner = current_running[cpu_id]->pid;
        current_running[cpu_id]->lock[current_running[cpu_id]->lock_num] = &(mlocks[mlock_idx]);
        current_running[cpu_id]->lock_num = current_running[cpu_id]->lock_num + 1;
    }
    else 
    {
...
void do_mutex_lock_release(int mlock_idx)
{
    ...
    for(int i = 0; i < current_running[cpu_id]->lock_num; i++)
    {
        ...
        {
            for(int j = i+1; j < current_running[cpu_id]->lock_num; j++)
            {
                current_running[cpu_id]->lock[j-1] = current_running[cpu_id]->lock[j];
            }
            current_running[cpu_id]->lock_num--;
            current_running[cpu_id]->lock[current_running[cpu_id]->lock_num] = NULL;
            break;
        }
    }
    ...
    if(list_is_empty(&(mlocks[mlock_idx].block_queue)))
    {
        ...
        mlocks[mlock_idx].lock.owner = 0;
    }
    else 
    {
        pcb_t *pcb_to_get_lock = do_unblock(mlocks[mlock_idx].block_queue.next);
        mlocks[mlock_idx].lock.owner = pcb_to_get_lock->pid;
        pcb_to_get_lock->lock[pcb_to_get_lock->lock_num] = &(mlocks[mlock_idx]);
        pcb_to_get_lock->lock_num = pcb_to_get_lock->lock_num + 1;
    }
}
