...
static void create_image(int nfiles, char *files[])
{
    ...
    for (int fidx = 0; fidx < nfiles; ++fidx) {
        ...
        for (int ph = 0; ph < ehdr.e_phnum; ph++) {
            ...
            if(taskidx >= 0)
            {
                taskinfo[taskidx].task_memsz += phdr.p_memsz;
            }
        }
                           ...
                           short tasknum, FILE * img)
{
    ...
    /*写入taskinfo_offset*/
    //int task_info_offset = (taskinfo[tasknum-1].task_offset + taskinfo[tasknum-1].task_size) / SECTOR_SIZE;
    int task_info_offset = 1000;
    printf("task_info_offset:%d\n", task_info_offset);
    fseek(img, TASK_INFO_OFFSET_LOC, SEEK_SET);
    fwrite(&task_info_offset, sizeof(task_info_offset), 1, img);
    printf("%s\n", taskinfo[16].task_name);
